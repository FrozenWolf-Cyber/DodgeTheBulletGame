...
def send_player_game_data(player_health,player_score,obstacleles_list_above,obstacleles_list_side,playerCar,boosters):
    file = open("game_data.txt",'a')

    file.write(str(player_health)+" " + str(player_score)+"\n")

    for i in obstacleles_list_above:
        file.write(str(i.rect.x)+" "+str(i.rect.y)+" "+str(i.speed)+" "+str(i.angle)+" ")
...
def game_manage(all_sprites_list,obstacleles_list_side,obstacleles_list_above,boosters_list):
    ...
    if len(obstacleles_list_side)<5:
        obstacle = bullet(RED, 15,15, random.choice([1,799]),random.randrange(1,800),random.randrange(0,360),random.randrange(2,5))
        obstacleles_list_side.add(obstacle)
    if len(obstacleles_list_side)<5:
        obstacle = bullet(RED, 15,15, random.randrange(1,800),random.choice([1,799]),random.randrange(0,360),random.randrange(2,5))
...
while carryOn:
        ...
        game_manage(all_sprites_list,obstacleles_list_side,obstacleles_list_above,boosters)
        ...
        send_player_game_data(player_health,player_score,obstacleles_list_above,obstacleles_list_side,playerCar,boosters)
